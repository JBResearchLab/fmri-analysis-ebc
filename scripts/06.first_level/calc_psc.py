...
def calc_psc_workflow(projDir, derivDir, resultsDir, workDir, sub, ses, task, sub_runs, contrast_opts, splithalf_id, hpf, filter_opt, TR, space_name, name='{}_task-{}_calcpsc'):
    ...
    subworkDir = op.join(workDir, 'sub-{}_task-{}_calcpsc'.format(sub, task))
    ...
    for r in sub_runs:
        ...
        if splithalf_id == 0:
            ...
            denoised_file = op.join(pscDir, 'sub-{}_task-{}_run-{:02d}_denoised.nii.gz'.format(sub, task, r))
            
            # define output run mean file - optional output for data checking
            mean_file = op.join(pscDir, 'sub-{}_task-{}_run-{:02d}_voxelwise_mean.nii.gz'.format(sub, task, r))
            ...
            # save intermediate file - optional output for data checking
            denoised_file = op.join(pscDir, 'sub-{}_task-{}_run-{:02d}_splithalf-{:02d}_denoised.nii.gz'.format(sub, task, r, splithalf_id))
            
            # define output run mean file - optional output for data checking
            mean_file = op.join(pscDir, 'sub-{}_task-{}_run-{:02d}_splithalf-{:02d}_voxelwise_mean.nii.gz'.format(sub, task, r, splithalf_id))
        ...
        if not scaled_file:
            raise FileNotFoundError('No scaled firstlevel outputs found for sub-{}'.format(sub))
