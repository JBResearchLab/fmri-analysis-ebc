...
def calc_psc_workflow(projDir, resultsDir, smoothDir, workDir, sub, ses, task, sub_runs, contrast_opts, splithalf_id, name='sub-{}_task-{}_calcpsc'):
    ...
    for r in sub_runs:
        ...
        if smoothDir:
            preprocDir = op.join(smoothDir, 'sub-{}'.format(sub), 'preproc', 'run{}'.format(r))
        else:
            preprocDir = op.join(resultsDir, 'sub-{}'.format(sub), 'preproc', 'run{}'.format(r))
