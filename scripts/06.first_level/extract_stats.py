...
def process_subject(projDir, sharedDir, resultsDir, froiDir, sub, runs, folds, task, contrast_opts, splithalves, mask_opts, match_events, template, extract_opt, psc, top_nvox, percent):
    ...
    if froiDir == None:
        ...
        stats_file = op.join(statsDir, 'sub-{}_task-{}_{}_{}_ROI_magnitudes.csv'.format(sub, task, localiser_name, extract_opt))
    ...
    for run_id in runs_inter:
        ...
        if folds == 'yes':
            combinedDir = op.join(resultsDir, 'sub-{}'.format(sub), 'model', 'combined_runs', 'fold{}'.format(run_id))
            ...
            for m in mask_opts:
                ...
                if 'fROI' in m:
                    ...
                    if splithalf_id != 0 and combined == 'no':
                        ...
                        mni_file = glob.glob(op.join(resultsDir, 'sub-{}'.format(sub), 'preproc', 'run{}_splithalf{}'.format(run_id, splithalf_id), '*_bold.nii.gz'))[0]
                        ...
                        mni_file = glob.glob(op.join(resultsDir, 'sub-{}'.format(sub), 'preproc', 'run{}_splithalf{}'.format(run_id, splithalf_id), '*_bold.nii.gz'))[0]
                        if folds == 'yes':
                            modelDir = op.join(resultsDir, 'sub-{}'.format(sub), 'model', 'combined_runs', 'splithalf{}'.format(splithalf_id), 'fold{}'.format(run_id))
                        else:
