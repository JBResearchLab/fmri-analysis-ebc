                    ...
                    if not op.exists(froi_prefix):
                        ...
                        if len(roi_file) > 1:
                            ...
                            if percent == 'yes':
                                ...
                                roi_file = glob.glob(op.join('{}'.format(froi_prefix),'sub-{}_*{}_*{}pc_*.nii.gz'.format(sub, roi_name, top_nvox)))                            
                            # if x% not indicated in config file, look for the file that matches the number of voxels specified in config file
                            else:
                                print('Multiple {} fROIs found. Using the file with {} top voxels.'.format(roi_name, top_nvox))
                                roi_file = glob.glob(op.join('{}'.format(froi_prefix),'sub-{}_*{}_*top{}.nii.gz'.format(sub, roi_name, top_nvox)))
